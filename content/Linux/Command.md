...
## uniq
过滤掉相邻的重复行，**重复行需要相邻(需要先用sort排序，使得重复行相邻)。** 比如 `sort /home/test | uniq` 输出去除重复行的内容
...
`top -p 20348` 监控进程号为`20348`的进程信息
- \-Hp pid 实时监控进程号为`pid`的**线程**状态
`top -Hp 20348` 监控进程号为`20348`的进程内部的线程状态
- \-d seconds 每seconds秒刷新一次

## du
查看某个目录中所有文件大小。 比如 `du js -h` 查看js目录中文件占用空间大小。
### 参数
- \-c 最后列出这个目录的总大小
- \-h 可读性更好一些
- \-s 只输出当前目录的大小，不会递归输出子目录的大小。
...
## sar
监控CPU使用率、内存使用率、磁盘IO情况、网卡工作情况 比如 `sar -b 1 2` 查看磁盘IO情况，每一秒显示一次，共显示两次。(需要开启系统监控: 修改`/etc/default/sysstat`中的`ENABLED=true`)
### 参数
...
- -H 指定请求头 -X指定请求方法
    ...
    ` curl -H 'Content-Type: application/json' -XPOST https://www.baidu.com`
...
扫描服务器端口信息， 比如 `nmap ip -p port` 查看指定IP的指定端口打开状态 或者 `nmap ip` 查看指定ip机器打开的全部端口 

###  参数

- \-p 指定端口（默认是TCP）

    `nmap 114.114.114.114 -p 23` 扫描23端口， `nmap 114.114.114.114 -p 1-1023` 扫描1到1024端口
...
执行定时任务，可以以固定的时间间隔执行一些命令或者脚本文件。具体用法： crontab [提前写好的定时文件]
...
- \-F 会在末尾加 **/** 标志是文件还是目录  
- \-R 递归打印每个目录  
- 通过 *、?、[a-i]、[!a] 实现过滤器。 比如`ls -l da[!t]a`列出第三个字符不是`t`的目录中的内容  
- \-i 查看文件的inode编号  

## cp 
拷贝文件
### 参数
- \-i 在拷贝的时候会判断新目录中是否已经存在同名文件，如果存在会询问。否则就会覆盖文件内容。比如 `cp -i test.md /home/js/` 如果`/homg/js/` 里面已经有`test.md`，就会提示。 `mv`/`rm`命令同理  
- 注意: 当将文件拷贝到一个不存在的目录时，如果在最后加上 `/`。 比如 `cp test.md /home/sj` 如果`sj`不存在并且后面还没有`/` 就会创建`sj`文件，并且将`test.md`的内容拷贝到`sj`中。如果在`sj`后面加上`/` 如果没有`sj`目录，就会提示错误信息，需要对比的是，如果将一个目录递归拷贝到另一个不存在的目录就可以实现。
...
- \-xf ：解包(extract)（打包的逆操作）

- \-z : 在打包/解包的同时，通过gzip进行压缩/解压

- \-tf 或者 --list: 列出(list)这个包中的内容

- \-rf 或者 --append: 往包中追加文件
...
- rm 删除文件/文件夹
- mv 移动文件/文件夹、修改文件名
- less 分页查看文件内容。\-N显示行号。 其它还有cat、tac
